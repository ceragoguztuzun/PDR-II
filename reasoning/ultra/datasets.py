...
class TransductiveDataset(InMemoryDataset):
    ...
    def classify_entity(self, entity_str: str) -> str:
        # disease nodes: "C" + 7 digits, e.g., "C0852654"
        if re.match(r'^C\d{7}$', entity_str):
            return "disease"

        # drug nodes: "DB" + 5 digits, e.g., "DB00303"
        if re.match(r'^DB\d{5}$', entity_str):
            return "drug"

        # phenotype nodes: "MP:" or "HP:", e.g., "MP:0005389", "HP:0002342"
        if entity_str.startswith("MP:") or entity_str.startswith("HP:"):
            return "phenotype"

        # anything else => gene (by your definition)
        return "gene"

    
    def load_file(self, triplet_file, inv_entity_vocab={}, inv_rel_vocab={}, entity_type_map=None):
        if entity_type_map is None:
            entity_type_map = {}  # ID -> type
        ...
        with open(triplet_file, "r", encoding="utf-8") as fin:
            for line in fin:
                parts = line.strip().split(self.delimiter) if self.delimiter else line.split()
                if len(parts) != 3:
                    print("Skipping malformed line:", parts)
                    continue
                u_str, r_str, v_str = parts

                # -- Build or look up IDs for the entities/relations --
                if u_str not in inv_entity_vocab:
                    inv_entity_vocab[u_str] = entity_cnt
                    # also record the type for this new entity
                    entity_type_map[entity_cnt] = self.classify_entity(u_str)
                    entity_cnt += 1

                if v_str not in inv_entity_vocab:
                    inv_entity_vocab[v_str] = entity_cnt
                    # also record the type for this new entity
                    entity_type_map[entity_cnt] = self.classify_entity(v_str)
                    entity_cnt += 1

                if r_str not in inv_rel_vocab:
                    inv_rel_vocab[r_str] = rel_cnt
                    rel_cnt += 1

                u_id = inv_entity_vocab[u_str]
                r_id = inv_rel_vocab[r_str]
                v_id = inv_entity_vocab[v_str]

                triplets.append((u_id, v_id, r_id))

        print(
            f"Loaded {len(triplets)} triplets from {triplet_file} ...\n"
            f"  num_node={len(inv_entity_vocab)}, num_relation={rel_cnt}"
        )
        ...
        return {
            ...
            "entity_type_map": entity_type_map  # <--- store this
        }
    ...
    def process(self):
        ...
        valid_results = self.load_file(
            ...
            train_results["entity_type_map"]
        )
        test_results = self.load_file(
            train_files[2],
            valid_results["inv_entity_vocab"],
            valid_results["inv_rel_vocab"],
            valid_results["entity_type_map"]
        )

        # Keep the final updated type map
        entity_type_map = test_results["entity_type_map"]
        ...
        # ------------------------------------------------------------
        # 1) Convert your `entity_type_map` (ent_id->string) into a
        #    numeric tensor, e.g. 0=disease, 1=drug, 2=phenotype, 3=gene
        # ------------------------------------------------------------
        type_codes = {
            'disease': 0,
            'drug': 1,
        ...
        # Initialize a list for all nodes
        entity_type_list = [0] * num_node

        for ent_id, ent_type_str in entity_type_map.items():
            entity_type_list[ent_id] = type_codes[ent_type_str]

        entity_type_tensor = torch.tensor(entity_type_list, dtype=torch.long)

        # Attach this tensor to each split if you like
        train_data.entity_type = entity_type_tensor
        valid_data.entity_type = entity_type_tensor
        test_data.entity_type = entity_type_tensor
        ...
        # ------------------------------------------------------------
        # 2) Save everything to data.pt
        # ------------------------------------------------------------
        torch.save((self.collate([train_data, valid_data, test_data])), self.processed_paths[0])

        # ------------------------------------------------------------
        # The rest: save dictionaries for your separate .pt file
        # ------------------------------------------------------------
        dicts_path = os.path.join(self.processed_dir, "entity_relation_dicts.pt")

        rel_dict_forward = test_results["inv_rel_vocab"]
        entity_dict_forward = test_results["inv_entity_vocab"]

        rel_dict_inverted = {v: k for k, v in rel_dict_forward.items()}
        entity_dict_inverted = {v: k for k, v in entity_dict_forward.items()}
        ...
        for ent_id, ent_str in entity_dict_inverted.items():
            ent_type = entity_type_map[ent_id]
            if ent_type == "disease":
                disease_inv_vocab[ent_str] = ent_id
            elif ent_type == "drug":
                drug_inv_vocab[ent_str] = ent_id
            elif ent_type == "phenotype":
                phenotype_inv_vocab[ent_str] = ent_id
            else:
                gene_inv_vocab[ent_str] = ent_id

        torch.save({
            "entity_dict_forward": entity_dict_forward,
            "relation_dict_forward": rel_dict_forward,
            "entity_dict_inverted": entity_dict_inverted,
            "relation_dict_inverted": rel_dict_inverted,
            "entity_type_map": entity_type_map,
            "disease_inv_vocab": disease_inv_vocab,
            "drug_inv_vocab": drug_inv_vocab,
            "phenotype_inv_vocab": phenotype_inv_vocab,
            "gene_inv_vocab": gene_inv_vocab
        }, dicts_path)

        print(f"Saved entity and relation dictionaries + type maps to {dicts_path}")
    ...
    def process(self):
        ...
        train_results = self.load_file(train_files[0], inv_entity_vocab={}, inv_rel_vocab={}, entity_type_map=entity_type_map)
        valid_results = self.load_file(train_files[1], train_results["inv_entity_vocab"], train_results["inv_rel_vocab"], train_results["entity_type_map"])
        test_results = self.load_file(train_files[2], valid_results["inv_entity_vocab"], valid_results["inv_rel_vocab"], valid_results["entity_type_map"])#train_results["inv_entity_vocab"], train_results["inv_rel_vocab"])
        ...
        test_data.entity_type_map = entity_type_tensor  # or a dictionary
        ...
        # if you want to build type-specific dictionaries in string form:
        # (i.e., "disease_inv_vocab", "drug_inv_vocab", etc.)
        disease_inv_vocab = {}
        drug_inv_vocab = {}
        phenotype_inv_vocab = {}
        gene_inv_vocab = {}

        # Let's invert "entity_dict_forward" so int->string, then read from `entity_type_map`
        # We already have `entity_dict_inverted = {id -> string}`, so we can do:
        for ent_id, ent_str in entity_dict_inverted.items():
            ent_type = entity_type_map[ent_id]
            if ent_type == "disease":
                disease_inv_vocab[ent_str] = ent_id
        ...
        torch.save({
            ...
            # Also the separate string->ID dictionaries by type
            "disease_inv_vocab": disease_inv_vocab,
            "drug_inv_vocab": drug_inv_vocab,
            "phenotype_inv_vocab": phenotype_inv_vocab,
            "gene_inv_vocab": gene_inv_vocab
...
''' transductive GPKG '''
class NEW_GPKG_T(TransductiveDataset):

    urls = [
        
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/train.txt",#"https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/train.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/valid.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/test.txt",
    ]
    name = "new_GPKG_T_nonaugg_Ww"
    delimiter = "\t"

''' case study: transductive GPKG '''
class NEW_GPKG_T_AD(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/train.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/valid.txt",
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/ad_pre_new.tsv"
    ]
    name = "new_GPKG_T_AD_adremi_1111333"
    delimiter = "\t"

''' case study: transductive GPKG '''
class NEW_GPKG_T_CF(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/train.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/valid.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/cf_pre.tsv"
    ]
    name = "new_GPKG_T_CF__"
    delimiter = "\t"

''' case study: transductive GPKG '''
class NEW_GPKG_T_MS(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/train.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/valid.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/ms_pre.tsv"
    ]
    name = "new_GPKG_T_MS_____"
    delimiter = "\t"


''' case study: transductive GPKG '''
class NEW_GPKG_T_LC(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/train.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/valid.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/lc_pre.tsv"
    ]
    name = "new_GPKG_T_LC_____"
    delimiter = "\t"

''' case study: transductive GPKG '''
class NEW_GPKG_T_LC_COMMONN(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/aug/train_augmented_common_n.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/non_aug/transd/valid.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/lc_pre.tsv"
    ]
    name = "NEW_GPKG_T_LC_COMMONN_valentinoOLC"
    delimiter = "\t"


''' case study: transductive GPKG '''
class NEW_GPKG_T_LC_PROP(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/aug/train_augmented_propagation.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/non_aug/transd/valid.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/lc_pre.tsv"
    ]
    name = "NEW_GPKG_T_LC_PROP_valentinoOlco0"
    delimiter = "\t"


''' augmented: transductive GPKG '''
class NEW_GPKG_T_AUG(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/ablation/pos_removed/train_augmented_propagation.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/valid.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/test.txt",
    ]
    name = "new_GPKG_T_AUG_wWw_abl_um_pos_propagation"
    delimiter = "\t"

''' case study: augmented: transductive GPKG ''' 
class NEW_GPKG_T_AUG_AD(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/aug/train_augmented_propagation.txt",#"https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/aug/train_augmented_katz.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/transd/valid.txt",
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/ad_pre_new.tsv"
        
    ]
    name = "new_GPKG_T_AUG_AD_propagation_W"
    delimiter = "\t"

''' case study: augmented: transductive GPKG ''' 
class NEW_GPKG_T_AUG_MS(TransductiveDataset):
    ...
    name = "new_GPKG_T_AUG_MS_PROP"
    delimiter = "\t"

''' case study: augmented: transductive GPKG ''' 
class NEW_GPKG_T_AUG_CF(TransductiveDataset):
    ...
    name = "new_GPKG_T_AUG_CF_PROP"
    delimiter = "\t"

#############################################
''' transductive pKG '''
class KG4FM(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1272434/foundation/train.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1272434/foundation/valid.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1272434/foundation/test.txt"
    ]
    name = "biomedical_KG_4_FM"
    delimiter = "\t"

''' transductive pKG ''' #Usopp
class KG_USOPP(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1051341/fine_tuning/train.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1051341/fine_tuning/valid.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1051341/fine_tuning/test.txt"
    ]
    name = "biomedical_KG_1051341"
    delimiter = "\t"


''' transductive pKG ''' #Zoro
class KG_ZORO(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1083686/fine_tuning/train.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1083686/fine_tuning/valid.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1083686/fine_tuning/test.txt"
    ]
    name = "biomedical_KG_1083686"
    delimiter = "\t"


''' transductive pKG '''
class KG_ROBIN(TransductiveDataset): #Robin

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1102120/fine_tuning/train.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1102120/fine_tuning/valid.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/outputs/outputs_1102120/fine_tuning/test.txt"
    ]
    name = "biomedical_KG_1102120"
    delimiter = "\t"



''' transductive pKG '''
class pKG_T(TransductiveDataset):

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/patient_KGs_G20/patient_1272434_kg.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/patient_KGs_G20/patient_1272434_kg.txt", 
        "https://media.githubusercontent.com/media/ceragoguztuzun/patient_integrated_kgs/refs/heads/main/patient_KGs_G20/patient_1272434_kg.txt"
    ]
    name = "pKG_1272434"
...
class GPKG_T(TransductiveDataset):
    ...
    urls = [
        "https://www.dropbox.com/scl/fi/fer620tkom728cbe7pmhd/train.txt?rlkey=ls2ba3wacctqkq31jft7tyhm8&dl=1",
        "https://www.dropbox.com/scl/fi/tp83396yuendgn30cwkox/valid.txt?rlkey=dksczkmhkpfeq05sft6x3qw2v&dl=1",
        "https://www.dropbox.com/scl/fi/a8lk7f7smmzmugc8272jv/test.txt?rlkey=0b0ccsow3mpezrvk1nwry3ofg&dl=1",
    ]
    name = "GPKG_Tt"
    delimiter = "\t"

''' case study: transductive GPKG '''
class GPKG_T_AD(TransductiveDataset):

    urls = [
        "https://www.dropbox.com/scl/fi/fer620tkom728cbe7pmhd/train.txt?rlkey=ls2ba3wacctqkq31jft7tyhm8&dl=1",
        "https://www.dropbox.com/scl/fi/tp83396yuendgn30cwkox/valid.txt?rlkey=dksczkmhkpfeq05sft6x3qw2v&dl=1",
        "https://www.dropbox.com/scl/fi/iujupttdfwmapx6bqb7x5/ad_pre.txt?rlkey=rhxfnvnal8r47bwzo95vp2biy&dl=1", #ad_pre for case study test set
    ]
    name = "GPKG_T_ADd"
    delimiter = "\t"

''' augmented: transductive GPKG ''' #TODO
class GPKG_T_AUG(TransductiveDataset):

    urls = [
        #"https://www.dropbox.com/scl/fi/0mq1wh7xdvr8pofmyjrgh/train_augmented_common_neighbors.txt%26dl%3D1", #augmented COMMON_NEIHB data
        "https://www.dropbox.com/scl/fi/0mq1wh7xdvr8pofmyjrgh/train_augmented_common_neighbors.txt?rlkey=7n8o917cq71hwdumgtw9ob63w&dl=1", 
        #"https://www.dropbox.com/scl/fi/fer620tkom728cbe7pmhd/train.txt?rlkey=ls2ba3wacctqkq31jft7tyhm8&dl=1",
        "https://www.dropbox.com/scl/fi/tp83396yuendgn30cwkox/valid.txt?rlkey=dksczkmhkpfeq05sft6x3qw2v&dl=1",
        "https://www.dropbox.com/scl/fi/a8lk7f7smmzmugc8272jv/test.txt?rlkey=0b0ccsow3mpezrvk1nwry3ofg&dl=1",
    ]
    name = "gpkg_t_auggggGGG"
    delimiter = "\t"

''' case study: augmented: transductive GPKG ''' 
class GPKG_T_AUG_AD(TransductiveDataset):

    urls = [
        "https://www.dropbox.com/scl/fi/0mq1wh7xdvr8pofmyjrgh/train_augmented_common_neighbors.txt?rlkey=7n8o917cq71hwdumgtw9ob63w&dl=1", 
        "https://www.dropbox.com/scl/fi/tp83396yuendgn30cwkox/valid.txt?rlkey=dksczkmhkpfeq05sft6x3qw2v&dl=1",
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/ad_pre.txt", #ad_pre for case study test set
    ]
    name = "gpkg_t_aug_adije"
    delimiter = "\t"
...
class Hetionet(TransductiveDataset):
    ...
    name = "hetionet"
#https://www.dropbox.com/scl/fi/0mq1wh7xdvr8pofmyjrgh/train_augmented_common_neighbors.txt?dl=1
#https://www.dropbox.com/s/y47bt9oq57h6l5k/train.txt?dl=1
...
class InductiveDataset(InMemoryDataset):
    ...
    def load_file(self, triplet_file, inv_entity_vocab={}, inv_rel_vocab={}):
        ...
        self.delimeter = '\t' #i might wanna delete this later
        ...
        with open(triplet_file, "r", encoding="utf-8") as fin:
            ...
            for l in fin:
                ...
                # Ignore lines where parts has only 2 elements
                #if len(parts[:3]) == 0:
                #    print(parts[:3], '== 0')
    ...
    def process(self):
        ...
        #print('----->', inference_res["inv_entity_vocab"].keys())

        # Check for the item 2 in any of the values
        '''
        found_keys = []
        for key, value in inference_res["inv_entity_vocab"].items():
            if 7 == value:
                found_keys.append(key)

        # Check if we found any keys and print them
        if found_keys:
            print("Found the item 2 in keys:", found_keys)
        else:
            print("Item 2 not found in any value.")

        ##

        print('TREAT is:', inference_res["inv_entity_vocab"].get('treat'))
...
''' semi-inductive GPKG '''
class NEW_GPKG_SI(InductiveDataset): 

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/aug/train_augmented_propagation.txt", #train
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/infer_nonaug.txt", #inference graph (valid + test)
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/valid.txt", #valid
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/test.txt" #test
        ]
    name = "new_GPKG_SIi_propagation_Ww"


''' semi-inductive GPKG '''
class NEW_GPKG_SI_MS_PROP(InductiveDataset): 
    ...
    name = "NEW_GPKG_SI_MS_PROP_valentino"
...
class NEW_GPKG_SI_MS_PROP(InductiveDataset): 
    ...
    name = "NEW_GPKG_SI_MS_PROP_valentino0909SSSS"



''' semi-inductive GPKG '''
#NEW_GPKG_SI_MS_COMMONN
#NEW_GPKG_SI_LC_COMMONN
class NEW_GPKG_SI_LC_COMMONN(InductiveDataset): 

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/aug/train_augmented_common_n.txt",#train
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/infer_nonaug.txt", #inference graph (valid + test)
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/valid.txt", #valid
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/lc_pre.tsv"
        ]
    name = "NEW_GPKG_SI_LC_COMMONN___wutwut"


class NEW_GPKG_SI_LC_PROP(InductiveDataset): 

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/aug/train_augmented_propagation.txt",#train
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/infer_nonaug.txt", #inference graph (valid + test)
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/valid.txt", #valid
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/refs/heads/main/lc_pre.tsv"
        ]
    name = "NEW_GPKG_SI_LC_PROP___wutwSut"




''' augmented: semi-inductive GPKG '''
class NEW_GPKG_SI_AUG(InductiveDataset): 

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/ablation/pos_removed/train_augmented_katz.txt",#"https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/aug/train_augmented_propagation.txt", #train
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/infer_nonaug.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/valid.txt", #valid
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/test.txt" #test
        ]
    name = "new_GPKG_SI_AUG_abl_um_negi_katzi"


''' case-study: semi-inductive GPKG '''
class NEW_GPKG_SI_AD(InductiveDataset): 

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/train.txt", #train
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/infer_nonaug.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/valid.txt", #valid
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/ad_pre_new.tsv"
        ]
    name = "new_GPKG_SI_AD_letsgooooo00_wW"


''' case-study: semi-inductive MS GPKG '''
class NEW_GPKG_SI_CF(InductiveDataset): 
    ...
    name = "NEW_GPKG_SI_CF_cflol"



''' case-study: semi-inductive MS GPKG '''
class NEW_GPKG_SI_LC(InductiveDataset): 
    ...
    name = "NEW_GPKG_SI_LC_cflolL"


''' case-study: semi-inductive MS GPKG '''
class NEW_GPKG_SI_MS(InductiveDataset): 
    ...
    name = "NEW_GPKG_SI_MS__fr__"

''' case-study: augmented: semi-inductive GPKG '''
class NEW_GPKG_SI_AUG_AD(InductiveDataset): 

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/train.txt", #train
        #"https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/aug/train_augmented_propagation.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/infer_nonaug.txt",
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/non_aug/semi_ind/valid.txt", #valid
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/ad_pre_new.tsv"
        ]
    name = "new_GPKG_SI_AUG_AD__um_zero_simmy"


###########################################################
''' semi-inductive GPKG '''
class GPKG_SI(InductiveDataset): 

    urls = [
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/train.txt?token=GHSAT0AAAAAACMMXWZIYJFSDNPJ4VVXPJ2QZOT2NDA", #train
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/inf.txt?token=GHSAT0AAAAAACMMXWZJEX6DFLK2DQIJVIYSZOT2LDQ", #inference graph (valid + test)
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/valid.txt?token=GHSAT0AAAAAACMMXWZJE27GCRXPCW6PZV3SZOT2KUQ", #valid
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/test.txt?token=GHSAT0AAAAAACMMXWZJWXVAUIT5X7JNO634ZOT2LLA" #test
        ]
    name = "GPKG_SIndi"

''' case-study: semi-inductive GPKG '''
class GPKG_SI_AD(InductiveDataset): 

    urls = [
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/train.txt", #train
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/inf.txt", #inference graph (valid + test)
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/valid.txt", #valid
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/ad_pre.txt" #ad_pre for case study test set
        ]
    name = "GPKG_Sind_AD"

''' case-study: augmented: semi-inductive GPKG '''
class GPKG_SI_AUG_AD(InductiveDataset): 

    urls = [
        "https://media.githubusercontent.com/media/ceragoguztuzun/GPKG/main/train_augmented_common_neighbors.txt",
        #"https://www.dropbox.com/scl/fi/0mq1wh7xdvr8pofmyjrgh/train_augmented_common_neighbors.txt?rlkey=7n8o917cq71hwdumgtw9ob63w&dl=1", #train
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/inf.txt", #inference graph (valid + test)
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/valid.txt", #valid
        "https://raw.githubusercontent.com/ceragoguztuzun/GPKG/main/ad_pre.txt", #ad_pre for case study test set
        ]
    name = "GPKG_SI_AUG_Adjjiijj"
