...
class BaseNBFNet(nn.Module):
    ...
    def remove_easy_edges(self, data, h_index, t_index, r_index=None):
        ...
        data.num_relations = (data.edge_type.max() + 1).item()  # Convert to Python int
        num_rel_half = data.num_relations // 2  # a Python int
        r_index_ext = torch.cat([r_index, r_index + num_rel_half], dim=-1)
    ...
    def negative_sample_to_tail(self, h_index, t_index, r_index, num_direct_rel):
        # Ensure they all live on the same device
        device = h_index.device  # or r_index.device, whichever you prefer

        # If h_index, t_index, r_index might be on CPU, move them to 'device':
        h_index = h_index.to(device)
        t_index = t_index.to(device)
        r_index = r_index.to(device)
        
        # If num_direct_rel is an int, you can safely use it in r_index + num_direct_rel
        # and PyTorch will keep it on the GPU. If it's a tensor, move it too:
        if isinstance(num_direct_rel, torch.Tensor):
            num_direct_rel = num_direct_rel.to(device)

        # Now do your calculation:
        # is_t_neg: shape [B, 1] or something similar
        is_t_neg = (h_index == h_index[:, [0]]).all(dim=-1, keepdim=True)  # bool mask
        # 'is_t_neg' should be on the same device as 'h_index'—if not, do:
        is_t_neg = is_t_neg.to(device)
